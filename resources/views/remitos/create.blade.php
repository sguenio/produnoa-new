@extends('layouts.app')
@section('title', 'Registrar Remito')
@section('breadcrumbs')
...
@section('content')
    <div class="p-4 sm:p-6 bg-gray-800 shadow-lg rounded-lg max-w-4xl mx-auto">
        <h1 class="text-2xl font-bold text-slate-100 mb-6">Registrar Nuevo Remito</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select id="proveedor_id" name="proveedor_id" required
                        ...
                        @foreach ($proveedores as $proveedor)
                            <option value="{{ $proveedor->id }}"
                                {{ old('proveedor_id') == $proveedor->id ? 'selected' : '' }}>{{ $proveedor->nombre }}
                            </option>
                        @endforeach
                    </select>
                    @error('proveedor_id')
                ...
                <div>
                    <label for="codigo_remito" class="block text-sm font-medium text-slate-300 mb-1">Código de
                        Remito</label>
                    <input type="text" id="codigo_remito" name="codigo_remito" value="{{ old('codigo_remito') }}"
                        required
                        class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-md focus:ring-1 focus:ring-red-500 focus:border-red-500"
