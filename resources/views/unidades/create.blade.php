    ...
    <div class="p-4 sm:p-6 bg-gray-800 shadow-lg rounded-lg max-w-2xl mx-auto">
        ...
        <form action="{{ route('unidades.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="abreviatura" class="block text-sm font-medium text-slate-300 mb-1">Abreviatura</label>
                    <input type="text" id="abreviatura" name="abreviatura" value="{{ old('abreviatura') }}" required
                        class="w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-md focus:ring-1 focus:ring-red-500 focus:border-red-500"
                        placeholder="Ej: Kg">
                    @error('abreviatura')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
